...
if ( !empty($category) ) {
    foreach ( $category as $key => $val ) {
        ...
        //$sql = "SELECT `brand_id` FROM ".$GLOBALS['ecs']->table('goods') ." left join  WHERE ecs_goods.`cat_id` = 3  GROUP BY ecs_goods.`brand_id`";

        $sql = "SELECT  b.* FROM ".$GLOBALS['ecs']->table('goods') ."   LEFT JOIN ".
            $GLOBALS['ecs']->table('brand') ."  AS b ON b.`brand_id` = ".
            $GLOBALS['ecs']->table('goods') .".`brand_id`  WHERE ".
            $GLOBALS['ecs']->table('goods') .".`cat_id` = ".$val['id']."  AND  b.`brand_name` IS NOT NULL  GROUP BY ".
            $GLOBALS['ecs']->table('goods') .".`brand_id` "
        ...
        foreach ($ret as $key=>$value )
        {
            ...
            $brand_tmp["brandName"] = $value['brand_name'];
            $brand_tmp["brandId"] = $value['brand_id'];
            $brand[] = $brand_tmp;
